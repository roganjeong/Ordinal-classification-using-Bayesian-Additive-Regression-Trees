...
void update_alphas(NumericVector& alphas, const int& K, const NumericVector& UBs, const NumericVector& LBs, const NumericMatrix diagonal_sigma_gamma)
{

    Environment TruncatedNormal = Environment::namespace_env("TruncatedNormal");
    Function rtmvnorm = TruncatedNormal["rtmvnorm"];

    NumericVector temp1(K-1);
    // NumericMatrix temp2 = (diagonal_sigma_gamma)*(1/sigma_gamma);
    
    temp1 = rtmvnorm(1, rep(0, K-1), diagonal_sigma_gamma, LBs, UBs); // R_NilValue
