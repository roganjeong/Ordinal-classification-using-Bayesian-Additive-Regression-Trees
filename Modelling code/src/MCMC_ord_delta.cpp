...
List MCMC_ord(
    ...
    const double sigma_gamma = 1.0; // try testing 1 and (2/K)^(1/2)
    // Rcout << 1.0 << std::endl;
    // Rcout << sigma_gamma << std::endl;
    NumericMatrix diagonal_sigma_gamma(K-1, K-1);
    ...
    for (int i = 0; i < K-1; i++)
    {
        for (int j = 0; j < K-1; j++)
        {
            if (i == j)
            {
                diagonal_sigma_gamma(i, j) = sigma_gamma;
            }
            else
            {
                diagonal_sigma_gamma(i, j) = 0;
            }
        }
    }
    // Rcout << diagonal_sigma_gamma << std::endl;
    ...
    for (int iter = 1; iter <= n_iter; iter++)
    {
        ...
        update_alphas(alphas, K, UBs, LBs, diagonal_sigma_gamma);
