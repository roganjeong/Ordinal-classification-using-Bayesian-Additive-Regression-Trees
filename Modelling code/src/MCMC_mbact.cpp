...
List MCMC_ord(
    ...
    NumericMatrix prob_mat(n_test, K);
    NumericMatrix cum_prob_mat(n_test, K);
    NumericMatrix latent_mat(n_test, K);
    double max_prob_cat;
    ...
    for (int iter = 1; iter <= n_iter; iter++)
    {
        ...
        update_Z(Z, Y, K, alphas, Tree);
        // initial_Z(Z, Y, K, alphas);
        // original : update_Z(Z, Y_trt, Tree)
        // sample_Z(_, iter - 1) = Z;
        ...
        if ((iter > burnt) && (iter % thin == 0))
        {
            ...
            for (int i = 0; i < n_test; i++)
            {
                ...
                for (int j = 0; j < K-1; j++)
                {
                    latent_mat(i,j) = alphas(j) - predicted_mu(i);
                }

                for (int j = 0; j < K-1; j++)
                {
                    NumericVector temp = pnorm(latent_mat(i,j), 0, 1);
                    cum_prob_mat(i,j) = temp(0);

                    if (j == 0)
                    {
                        prob_mat(i,j) = cum_prob_mat(i,j);
                    }
                    else if (j == K-1)
                    {
                        prob_mat(i,j) = 1 - cum_prob_mat(i,j);
                    }
                    else
                    {
                        prob_mat(i,j) = cum_prob_mat(i,j) - cum_prob_mat(i,j-1);
                    }
                }

                max_prob_cat = Rcpp::max(prob_mat(i,_));
                for (int j = 0; j < K; j++)
                {
                    if (prob_mat(i,j) == max_prob_cat)
                    {
                        predicted_Y(i) = j+1;
                        post_predicted(i, post_iter) = j+1;
                    }
                }
            }
    ...
    // }
    List L = List::create(
        Named("Z")                     = Z, // for checking nan
        Named("LBs")                   = LBs, // for checking nan
        Named("UBs")                   = UBs, // for checking nan
        Named("alphas")                = alphas, // for checking nan
