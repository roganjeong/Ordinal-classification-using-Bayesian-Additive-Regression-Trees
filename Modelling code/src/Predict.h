...
// Prediction (ordinal categories)
void DecisionTree::Predict_ord
(
    NumericMatrix& Tree,
    // const Numeric& Y,
    // const NumericVector& alphas,
    const NumericVector xcut[],
    const NumericMatrix& Xpred_test, // xpred_mult --> Xpred_test
    const int n_test // n_test instead of n
) {
    // predict for ordinal categories
    for (int i = 0; i < n_test; i++)
    {
        ...
        while (Terminal(idx) != 1)
        {
            ...
            int    value = Value(idx);
            double rule  = xcut[split](value);
            double xpred = Xpred_test(i, split);
            if (xpred < rule)
            {
                // move to left child node
                // then update pos and idx
