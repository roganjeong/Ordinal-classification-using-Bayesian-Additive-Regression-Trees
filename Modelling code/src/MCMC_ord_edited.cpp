...
List MCMC_ord(
    ...
    const int burn_in,
    const int thinning,
    const bool fixed_sigma_mu = true,
    ...
    // NumericVector sample_sigma_mu(n_iter);
    NumericVector post_sigma_mu(n_post);
    ...
    for (int iter = 1; iter <= n_iter; iter++)
    {
        ...
        if (fixed_sigma_mu != true)
        {
            sigma_mu   = Rcpp::sd(Z); // (Rcpp::max(Z) - Rcpp::min(Z))/(4*sqrt(m));
        }
        ...
        update_alphas(alphas, Tree, K, Y, accepted);
        ...
        if ((iter > burnt) && (iter % thin == 0))
        {
            ...
            if (fixed_sigma_mu != true)
            {
                // post_sigma_mu(post_iter) = sample_sigma_mu(iter - 1);
                post_sigma_mu(post_iter) = sigma_mu;
            }
    ...
    if (fixed_sigma_mu != true)
    {
        List L = List::create(
            Named("Y_test")                = Y_test,
            Named("acceptance_rate")       = accepted,
            // Named("sample_sigma_mu")       = sample_sigma_mu,
            Named("post_sigma_mu")         = post_sigma_mu,
