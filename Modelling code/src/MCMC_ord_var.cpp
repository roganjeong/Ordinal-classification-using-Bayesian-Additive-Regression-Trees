...
List MCMC_ord(
    ...
    NumericVector standard_deviations_Z(n_iter);
    ...
    for (int iter = 1; iter <= n_iter; iter++)
    {
        if (verbose)
        {
            if (iter == 1)
            {
                ...
                Rcout << "Standard Deviation = (" << sd_Z << ")" << std::endl;
        ...
        if ( iter == 1 )
        {
            standard_deviations_Z(iter - 1) = 1;
        ...
        {
            update_sd(sd_Z, Z);
            standard_deviations_Z(iter - 1) = sd_Z;

            
            update_Z(Z, Y, K, alphas, Tree, sd_Z); // original : update_Z(Z, Y_trt, Tree)
            sample_Z(_, iter - 1) = Z;
    ...
    List L = List::create(
        ...
        Named("standard_deviations_Z") = standard_deviations_Z,
